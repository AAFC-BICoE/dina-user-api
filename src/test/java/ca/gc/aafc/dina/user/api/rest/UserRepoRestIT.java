...
import ca.gc.aafc.dina.user.api.testsupport.fixtures.DinaUserFixture;
...
public class UserRepoRestIT extends BaseKeycloakRestIt {
  ...
  void patch_ReplaceGroup_GroupsReplaced() {
    ...
    DinaUserDto obj = DinaUserFixture.newUserDto().build();
    obj.setRolesPerGroup(Map.of("cnc", Set.of(DinaUserFixture.GUEST_ROLE), "amf", Set.of(DinaUserFixture.GUEST_ROLE)));
    ...
    sendGetWithAuth(token, id)
      ...
      .body("data.attributes.rolesPerGroup.ccfc", Matchers.contains(DinaUserFixture.GUEST_ROLE));
  }
